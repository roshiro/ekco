...
this._proxyFactory=new this.imports.ProxyFactory(this._appInstance.getDataItemFactory(),this._appInstance.getTypesManager(),this._viewDefinitions,this._eventDispatcher,this._appInstance.getAppBasePath(),this._appInstance.getPackageName());
...
W.Components.newComponent({name:"wixapps.integration.components.Icon",skinParts:{img:{type:"htmlElement"}},Class:{Extends:"mobile.core.components.base.BaseComponent",initialize:function(c,a,b){this.parent(c,a,b)
},_onAllSkinPartsReady:function(a){a.img.set("src",this._data.get("url"));
a.img.set("width",this._data.get("width"));
a.img.set("height",this._data.get("height"))
...
this._initialState=b.initialState
},_onAllSkinPartsReady:function(a){a.on.addEvent("click",this.toggle);
...
}else{k.addEvent("proxyDisplayed",function(l){if(l.className=="wixapps.integration.proxies.layout.ZoomLayoutProxy"){l.addEvent(Constants.WixAppEvents.APP_ZOOM_READY,function(){k.setWidth(k.getSkinPart("inlineContent").children[0].getSize().x);
...
this._inlineDiv.setStyles({height:this._def.getChildValue("packHeight")==false?"100%":"auto",position:"static"})
}},_getComponentHeight:function(){return this._componentLogic.getSkinPart("inlineContent").getComputedStyle("height")
},_getNaturalHeight:function(){return this._innerElement.getComputedStyle("height")
}}});
...
var a=b.createProxyFromItemDefinition(this._viewContext,j,0).proxy;
var e=i.getChildByIndex(1);
var h=b.createProxyFromItemDefinition(this._viewContext,e,1).proxy;
...
},_onDefinitionsChanged:function(a){if(this._def.getChildValue("class")){this._element.addClass(this._def.getChildValue("class"))
...
return this._createWixComponent("wixapps.integration.components.Icon",a)
},_getDefaultSkinName:function(){return"wixapps.integration.skins.IconSkin"
},_createRawData:function(){var a=this._viewContext.getValue();
return{type:"Icon",url:this._toAppImageUrl(a.src),width:a.width,height:a.height}
...
W.Classes.newClass({name:"wixapps.integration.proxies.LabelProxy",imports:[],traits:["wixapps.integration.proxies.traits.AppsLinkTrait"],Class:{Extends:"wixapps.integration.proxies.WProxy",createComponent:function(){this.useSkinsInsteadOfStyles();
var b=this._createRawData();
...
if(a){b+=a
}if(this._viewContext.getTypeName()=="wix:RichText"){b+=this._viewContext.getChildByRef("text").getValue()
}else{b+=this._viewContext.getValue()
}if(c){b+=c
}return b
...
a.registerProxy("RichTextEditorInline",this.imports.RichTextEditorInlineProxy,[{forType:"wix:RichText",forMode:"edit"}]);
...
a.registerProxy("Video",this.imports.VideoProxy,[{forType:"Video",forMode:"view"},{forType:"wix:Video",forMode:"view"}]);
a.registerProxy("VideoSelector",this.imports.VideoSelectorProxy,[{forType:"Video",forMode:"edit"},{forType:"wix:Video",forMode:"edit"}]);
...
a.registerProxy("ZoomLayout",this.imports.ZoomLayoutProxy);
a.registerProxy("FixedRatioLayout",this.imports.FixedRatioLayoutProxy)
}}});
W.Classes.newClass({name:"wixapps.integration.proxies.RichTextEditorInlineProxy",imports:[],traits:["wixapps.integration.proxies.traits.AppsLinkTrait"],Class:{Extends:"wixapps.integration.proxies.WProxy",Binds:["_onComponentDataChanged","generateLinkId"],createComponent:function(){var b=this._createRawData();
var a=W.Data;
if(W.Editor){a=W.Preview.getPreviewManagers().Data
}this._updateLinkDataItems(a);
return this._createWixComponent("wysiwyg.editor.components.richtext.WRichTextEditor",b,{fixedHeight:this._def.getChildValue("height"),persistantLinks:false,generateLinkIdFunc:this.generateLinkId})
},_CHARS:"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".split(""),generateLinkId:function(){var f=this._CHARS,d=[],c=0,e;
for(var b=0;
b<8;
...
return this._viewContext.getEnvironment().getProxyFactory().getNameSpace()+"_"+a
},_getDefaultSkinName:function(){return"wysiwyg.editor.skins.WRichTextEditorSkin"
},_getDefaultStyleName:function(){return"wrte1"
},_createRawData:function(){var a=this._viewContext.getChildValue("text");
return{type:"RichText",text:a}
...
}this._viewContext.getChildByRef("links").setValue(b);
this._viewContext.getChildByRef("text").setValue(f)
},_onDefinitionsChanged:function(){},_onDataChanged:function(a){if(this._componentLogic){this._componentLogic.getDataItem().set("text",this._viewContext.getChildValue("text"))
}}}});
...
var f=this._viewContext.evaluatePath(this._def.getChildValue("expression")||"this");
var a=f?f.getValue().toString():"";
...
W.Classes.newClass({name:"wixapps.integration.proxies.ToggleProxy",imports:[],Class:{Extends:"wixapps.integration.proxies.WProxy",createComponent:function(){var b=this._viewContext.getEnvironment().getProxyFactory();
var e=this._def.getChildByRef("items");
var c=this.getViewDefinition().getChildValue("initialState");
if(e.getChildren().length!=2){throw"Expander must have two children, for selected and unselected states."
}var h=e.getChildByIndex(0);
var d=b.createProxyFromItemDefinition(this._viewContext,h,0).proxy;
var g=e.getChildByIndex(1);
var a=b.createProxyFromItemDefinition(this._viewContext,g,1).proxy;
this._childrenLeftToBeReady=2;
var i=function(j){j.grab(d.createComponent());
d.setupProxy(this._onChildReady)
}.bind(this);
var f=function(j){j.grab(a.createComponent());
a.setupProxy(this._onChildReady)
}.bind(this);
return this._createWixComponent("wixapps.integration.components.Toggle",undefined,{createOnHook:i,createOffHook:f,initialState:c})
...
},refreshSize:function(){},_onDefinitionsChanged:function(d){if(this._element){this._applyCssNode(this._element)
...
if(c){var d=e.payload.ref||e.payload.target.getParent().getChildRef(e.payload.target);
if(d){a.push(d)
}}})
}return a
},_applyCssNode:function(a){var b=this._def.getChildValue("css");
if(b){this._applyStyle(a,b)
}},_applyStyle:function(c,d){if(!c){return
}var a={};
var b;
...
},_onSkinChange:function(a){},_onStylePropertiesChange:function(a){},_processCssProp:function(b){var a;
...
}if(this._componentLogic){this._componentLogic.removeEvent("autoSizeChange",this._onAutoSizeChange)
...
var c=l[b]!==undefined?l[b]:(l.spacer||0);
var n=this._spacers[g]||{};
if(!d){c=0
}else{if(d.getViewProxy){var e=d.getViewProxy().getViewDefinition().getChildValue("hidden");
if(e===true||e==="true"){c=0
}}}if(n[k]&&(c===0)){this._element.removeChild(n[k]);
delete n[k]
}else{if(!n[k]&&c){var f=this._viewContext.getEnvironment();
var j={comp:{name:this._orientation=="VBox"?"VSpacer":"HSpacer"},layout:{}};
...
h.updateSize=function(p){a.getChildByRef("comp").getChildByRef("size").setValue(p)
};
a.getChildByRef("comp").addChild(c,"size");
n[k]=h;
if(k=="before"){this._element.insertBefore(h,this._components[g])
}else{var m=this._components[g].nextSibling;
if(m){this._element.insertBefore(h,m)
}else{this._element.appendChild(h)
}}var o=a.getChildValue("layout");
this._applyStyle(h,o);
this._processBoxFlex(h,o);
i.setupProxy()
}else{if(!n[k]&&!c){return
}else{n[k].updateSize(c)
}}}this._spacers[g]=n
...
W.Classes.newClass({name:"wixapps.integration.proxies.layout.FixedRatioLayoutProxy",imports:[],traits:["wixapps.integration.utils.FlexBoxIESupport"],Class:{Extends:"wixapps.integration.proxies.layout.CssLayoutProxy",Binds:["_onAppResize"],_setupBox:function(a){this.parent();
this.addEvent(Constants.WixAppEvents.APP_PART_RESIZE,this._onAppResize);
this.addEvent(Constants.WixAppEvents.APP_VIEW_READY,this._onAppResize)
},_onAppResize:function(){var c=1;
var d=this._def.getChildValue("aspectRatio")||1;
c=parseFloat(d);
if(d=="golden"){c=1.61803
}var b=this._element.getSize();
var a=Math.floor(b.x/c);
this._element.setStyles({height:a})
}}});
...
c._deactivateChildResizeMechanism(e);
d=Math.max(parseInt(g*0.5),parseInt(f*0.5));
b=Math.max(parseInt(g),parseInt(f));
c._setElementSize(e,d,d);
window.requestAnimFrame(function(){var i=a.getSize();
c._setElementSize(e,b,b);
window.requestAnimFrame(function(){var k=a.getSize();
var j=c._calculateSize(e,g,f,d,i,b,k);
c._approximateSize(a,e,g,f,{x:j.x,y:j.y},function(){c._activateChildResizeMechanism(e);
if(h){h()
}})
})
})
},_approximateSize:function(a,c,e,d,b,f){if(!isNaN(b.x)&&!isNaN(b.y)){this._setElementSize(c,b.x,b.y);
...
W.Classes.newTrait({name:"wixapps.integration.proxies.traits.AppsLinkTrait",trait:{_updateLinkDataItems:function(a){if(this._viewContext.getTypeName()!="wix:RichText"){return
}var b=this._viewContext.getChildValue("links");
for(var c=0;
c<b.length;
...
d.href="#!"+a.getDataByQuery("#"+c.pageId,function(){}).get("title")+"/"+c.pageId;
break;
...
d.href="mailto:"+c.email+"?subject="+c.subject;
d.text=c.subject;
break
}var b=a.addDataItem(c.linkId,d);
b.setMeta("isPersistent",true);
return b
...
W.Data.registerDataTypeSchema("Icon",{url:"string",width:"number",height:"number"});
...
},_ie_onResize:function(){if(this._element){if(this._orientation==="HBox"){}this._ie_recalcFlexes()
