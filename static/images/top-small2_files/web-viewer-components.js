...
}},_createIframe:function(){this._iframe=new IFrame({src:"about:blank",width:"100%",height:"100%",webkitAllowFullScreen:"true",mozallowfullscreen:"true",allowfullscreen:"allowfullscreen",frameborder:"0",scrolling:"no",style:{overflow:"visible"}});
this._setBasicIframeParams();
this._setIframeParams();
this._iframe.insertInto(this._getIframeContainer())
},_getIframeContainer:function(){return this._view
},_changeSize:function(c,a){var b=this.getWidth();
var f=this.getHeight();
var e=c.x||c.w||c.width;
var d=c.y||c.h||c.height;
if(e&&e!=b){this.setWidth(e,false,(!d&&a));
this._iframe.setStyle("width",e+"px")
}if(d&&d!=f){this.setHeight(d,false,a);
this._iframe.setStyle("height",d+"px")
}this._wCheckForSizeChangeAndFireAutoSized(1)
...
},_setIframeParams:function(){},_getUrl:function(a){throw new Error("IFrameComponent::_getUrl() must be overridden.")
...
},setGallery:function(d,c,b,a){if(!this._validateArgs(d,c)){return
}this._opened=true;
...
},_opened:false,isZoomOpened:function(){return this._opened
...
W.Components.newComponent({name:"wysiwyg.viewer.components.Modal",skinParts:{blockingLayer:{type:"htmlElement"},close:{type:"htmlElement"},innerComponent:{hookMethod:"_innerComponentDefinition"}},Class:{Extends:"mobile.core.components.base.BaseComponent",Binds:["_onCloseClick","_onBlockingLayerClick","_editModeChanged","_onViewportResize"],_states:{},initialize:function(c,a,b){this.parent(c,a,b);
b=b||{};
this._viewportSize=window.getSize();
this._innerComponentArgs=b.innerComponentArgs;
this._innerComponentsize=null
},render:function(){this.setSize();
if(this._skinParts.innerComponent.getTop()=="50%"){this._centerModal()
}},_onAllSkinPartsReady:function(a){this.parent();
window.addEvent(Constants.CoreEvents.RESIZE,this._onViewportResize);
a.close.addEvent(Constants.CoreEvents.CLICK,this._onCloseClick);
a.blockingLayer.addEvent(Constants.CoreEvents.CLICK,this._onBlockingLayerClick)
},_innerComponentDefinition:function(a){a.type=this._innerComponentArgs.type;
a.skin=this._innerComponentArgs.skin;
a.dataItem=this._innerComponentArgs.dataItem||this._data;
a.argObject=this._innerComponentArgs.argObject;
return a
},_onViewportResize:function(a){this._viewportSize=window.getSize();
this._renderIfReady()
},_onCloseClick:function(){this.dispose()
},_onBlockingLayerClick:function(){this.dispose()
},_editModeChanged:function(){this.dispose()
},_setClosePosition:function(){var b=this._skinParts.close;
var a=b.getSize();
b.setStyles({left:this._skinParts.innerComponent.getX()+this._skinParts.innerComponent.getWidth()-a.x/2,top:this._skinParts.innerComponent.getY()-a.y/2})
},_centerModal:function(){var d=this._skinParts.close;
var a=d.getSize();
var c=this._skinParts.innerComponent.getWidth()/2;
var b=this._skinParts.innerComponent.getHeight()/2;
this._skinParts.innerComponent.getViewNode().setStyles({"margin-left":-(c),"margin-top":-(b)});
d.setStyles({left:"50%",top:"50%","margin-left":c-a.x/2,"margin-top":-(b)-a.y/2})
},setSize:function(){var a,c,b,d;
...
b=this._innerComponentsize;
d=this._viewportSize;
a=Math.min(b.width,d.x);
c=Math.min(b.height,d.y);
if(this._skinParts.innerComponent.getWidth()!=a||this._skinParts.innerComponent.getHeight()!=c){this._skinParts.innerComponent.setWidth(a);
this._skinParts.innerComponent.setHeight(c,false,true)
}this._setClosePosition()
},dispose:function(){window.removeEvent(Constants.CoreEvents.RESIZE,this._onViewportResize);
if(this._skinParts.innerComponent){this._skinParts.innerComponent.dispose()
}this.collapse();
this.parent()
}}});
...
}else{var b=this._currentPage.getLogic().getDataItem().get("id");
window.scrollTo(0,0);
...
},_onResize:function(){if(this._initialized){this._renderIframe()
...
},_getUrlParams:function(){return{href:this._getLikeUrl(),layout:this._layout,show_faces:this._showFaces,action:this._action,colorscheme:this._colorScheme,send:this._send,width:this._getSizeAccordingToProperties().x}
},_getLikeUrl:function(){return location.href
...
if(this.injects().Viewer.getPreviewMode()){this._addBiEvent()
}},_addBiEvent:function(){var a=wixEvents.WIXADS_CLICKED_IN_PREVIEW;
this._skinParts.topRectLabel.addEvent("click",function(){LOG.reportEvent(a,{c1:"top-right"})
});
this._skinParts.footerLabel.addEvent("click",function(){LOG.reportEvent(a,{c1:"footer"})
})
},render:function(){this.parent();
...
W.Components.newComponent({name:"wysiwyg.viewer.components.tpapps.TPABase",traits:["wysiwyg.viewer.components.traits.TPAHandlers"],skinParts:{iframe:{type:"htmlElement"}},Class:{Extends:"wysiwyg.viewer.components.IFrameComponent",Binds:["_handlePageNavigationChange"],_renderTriggers:[Constants.DisplayEvents.DISPLAY_CHANGED],MIN_SIZE:{w:100,h:100},MAX_LOAD_RETRIES:10,EDITOR_META_DATA:{general:{settings:true,design:false},custom:[{label:"APPS_SETTINGS_APPS",command:"WEditorCommands.OpenTPASettingsDialog"}]},_Events:{EDIT_MODE_CHANGE:"WPreviewCommands.WEditModeChanged",PAGE_NAVIGATION_CHANGE:"pageTransitionEnded"},initialize:function(c,a,b){this.parent(c,a,b);
this._initialized=false;
this._appIsAlive=false;
this._loadingTries=0;
this.setMinW(this.MIN_SIZE.w);
this.setMinH(this.MIN_SIZE.h);
this.addEvent("resizeEnd",this._onResizeEnd)
},_onAllSkinPartsReady:function(){this.injects().TPA.reportLoadStart(this);
this.addEventListeners()
},setIFrameSrc:function(a){if(!a){this.injects().TPA.showAppNotAvailable(this)
}else{this.parent(a)
}},setUrl:function(a){this._iframe.src=a
},render:function(){this.parent()
},onDataInitialized:function(a){},onPageVisibilityChange:function(a,b){if(a){this.fireEvent("pageTransitionEnded",b._compId)
}},addEventListeners:function(){this.injects().Commands.registerCommandListenerByName(this._Events.EDIT_MODE_CHANGE,this,function(){this.handleAddEventListener.apply(this,[this._Events.EDIT_MODE_CHANGE])
});
this.injects().Viewer.addEvent(this._Events.PAGE_NAVIGATION_CHANGE,this._handlePageNavigationChange);
this.addEvent(this._Events.PAGE_NAVIGATION_CHANGE,this._handlePageNavigationChange)
},handleAddEventListener:function(c,d){var a;
var b={intent:"addEventListener",eventType:"",params:{}};
switch(c){case ("WPreviewCommands.WEditModeChanged"):a="EDIT_MODE_CHANGE";
b.params={editMode:this._getCurrentEditMode()};
break;
case ("pageTransitionEnded"):a="PAGE_NAVIGATION_CHANGE";
this._setPageParameters(d);
b.params={pageName:this._pageName,pageUrl:this._pageUrl,pageId:this._pageId};
break
}b.eventType=a;
if(this._skinParts.iframe){this._skinParts.iframe.contentWindow.postMessage(JSON.stringify(b),"*")
...
}},dispose:function(){this.injects().Viewer.removeEvent(this._Events.PAGE_NAVIGATION_CHANGE,this._handlePageNavigationChange);
this.injects().TPA.getComponentRegistrar().unregister(this);
this.parent()
},_handlePageNavigationChange:function(a){var c=$$("#"+a)[0].getLogic();
var b=c.getPageComponents().map(function(d){return d._compId
}).indexOf(this._compId)>-1;
if(this.getIsDisplayed()||b){this.handleAddEventListener(this._Events.PAGE_NAVIGATION_CHANGE,c)
}},_setPageParameters:function(a){this._pageName=a._data._data.title;
this._pageId=a._compId;
this._pageUrl=location.href
},_getCurrentEditMode:function(){if(window.top.W.Editor.getEditMode()=="PREVIEW"){return"preview"
}return"editor"
},_addCustomQueryString:function(a){if(this._queryParams){var b="";
Object.each(this._queryParams,function(d,c){b+="&"+c+"="+encodeURIComponent(d)
});
delete this._queryParams;
return a+b
}return a
},_onDataChange:function(a){this._initializeWithData()
},_extractAppData:function(){this._applicationId=this._data.get("applicationId");
...
if(this.isRendered()&&this._initialized){a={width:this.getWidth(),height:this.getHeight()};
this._changeSize(a,true)
}},_onResizeEnd:function(){if(this._appIsAlive&&this._skinParts){this._renderIfReady()
}this._wCheckForSizeChangeAndFireAutoSized(3)
},_createIframe:function(){this._iframe=this._skinParts.iframe;
this._setBasicIframeParams();
this._setIframeParams()
}}});
W.Components.newComponent({name:"wysiwyg.viewer.components.tpapps.TPAFixedWidget",Class:{Extends:"wysiwyg.viewer.components.tpapps.TPAWidget",Static:{DEFAULT_POSITION:{top:0,left:0,bottom:"auto",right:"auto"}},Binds:[],initialize:function(c,a,b){this.parent(c,a,b);
this._position=b.position||this.DEFAULT_POSITION;
this._$top="auto";
this._$left="auto";
this._$bottom="auto";
this._$right="auto"
...
var e=["x","y","width","height"];
var b=0;
for(b;
b<e.length;
b++){var d=e[b];
var c=this._view.get(d);
if(c!=null&&c!=""){var a="set"+d.capitalize();
this[a](c,true)
}}this._view.setStyle("position","fixed");
this.saveCurrentDimensions();
this.saveCurrentCoordinates()
},getLayoutMode:function(){return"FIXED"
},_setCssPosition:function(e,b){var a=(e=="auto"||(!e&&isNaN(e)));
var d=["top","left","bottom","right"];
var c="px";
if(!d.contains(b)){W.Utils.debugTrace("fixedPositionComponent","_setCssPostion",b+" passed as "+e);
return
}if(e&&e.split){c=e.split(parseInt(e,10)).join("");
if(c!=="%"){c="px"
}}e=(a)?"auto":parseInt(e,10);
if(this["_$"+b]!=e){if(!a&&isNaN(e)){W.Utils.debugTrace("fixedPositionComponent","set"+b,"NaN passed as value");
return
}this["_$"+b]=e+c;
this._view.setStyle(b,e+c);
if(b=="left"){this._$x=(a||c=="%")?this._view.getPosition().x:e
}if(b=="top"){this._$y=(a||c=="%")?this._view.getPosition().y:e
}this.onMovement()
}},setX:function(a){this.setLeft(a);
this.setRight("auto")
},setY:function(a){this.setTop(a);
this.setBottom("auto")
},getRight:function(){return this._$right
},setRight:function(a){this._setCssPosition(a,"right")
},getBottom:function(){return this._$bottom
},setBottom:function(a){this._setCssPosition(a,"bottom")
},getLeft:function(){return this._$left
},setLeft:function(a){this._setCssPosition(a,"left")
},getTop:function(){return this._$top
},setTop:function(a){this._setCssPosition(a,"top")
},setWidth:function(e,b,f){var d,c;
var a=e;
if(this._isPercent(e)){d=parseFloat(e)/100;
c=window.getSize().x;
a=Math.round(c*d)
}this.parent(a,b,f)
},setHeight:function(d,b,e){var c,f;
var a=d;
if(this._isPercent(d)){c=parseFloat(d)/100;
f=window.getSize().y;
a=Math.round(f*c)
}this.parent(a,b,e)
},setPosition:function(d,c,a,b){this.setTop(d);
this.setLeft(c);
this.setBottom(a);
this.setRight(b)
},setCoordinates:function(a){a=a||{};
this.setWidth(a.width||0,false,false);
this.setHeight(a.height||0,false,false);
this.setTop(a.top);
this.setLeft(a.left);
this.setBottom(a.bottom);
this.setRight(a.right)
},_onAutoSized:function(a){this.fireEvent("autoSizeChange")
},_isPercent:function(a){return(typeof a=="string"&&a.split(parseFloat(a)).join("")=="%")
}}});
W.Components.newComponent({name:"wysiwyg.viewer.components.tpapps.TPAPopup",Class:{Extends:"wysiwyg.viewer.components.tpapps.TPAFixedWidget",Binds:["_onResize"],initialize:function(c,a,b){this.parent(c,a,b);
b=b||{};
this._rawUrl=b.url;
this._url=null;
this._containingComponent=b.containingComponent;
this._coordinates=b.coordinates
},render:function(){this.setCoordinates(this._coordinates);
this.parent()
},_getUrl:function(){var a=this._url;
if(!a){a=W.TPA.getURLBuilder().buildStateLessUrl(this._rawUrl,["instance","section-url","target","width","viewMode"],this,this._appData)
}return a
},_onResize:function(){this.parent();
this._renderIfReady()
},handleClosePopup:function(){if(this._containingComponent){this._containingComponent.dispose()
}else{this.dispose()
}}}});
...
var b=this.injects().TPA.getURLBuilder().buildUrl(a,["instance","section-url","target","width","viewMode"],this,this._appData);
...
}},_getUrl:function(d){if(d||!this._url){var c=this._data.get("widgetId");
var e=this.injects().Viewer.getAppDataHandler().getWidgetData(this._applicationId,c);
var a=e.widgetUrl;
var b=this.injects().TPA.getURLBuilder().buildStateLessUrl(a,["instance","section-url","target","width","viewMode"],this,this._appData);
this._url=this._addCustomQueryString(b)
}return this._url
},handleAppStateChanged:function(){},handleClosePopup:function(){},setIFrameSrc:function(a){this._iframe.src=a;
this._url=this._iframe.src
},getAcceptableDataTypes:function(){return["TPAWidget",""]
}}});
...
W.Classes.newTrait({name:"wysiwyg.viewer.components.traits.TPAHandlers",trait:{Extends:"wysiwyg.viewer.components.traits.TPAHandlersBase",DEFAULTS:{POPUP_COORDINATES:{NORMAL:{top:"auto",left:"auto",bottom:"auto",right:"auto",width:500,height:500},MODAL:{top:"50%",left:"50%",bottom:"auto",right:"auto",width:500,height:500},FULLSCREEN:{top:0,left:0,bottom:0,right:0,width:"100%",height:"100%"}}},handleHeightChanged:function(a){this.handleChangeAppSize({height:a})
},handleChangeAppSize:function(a){if(this.injects().Viewer.isPublicMode()||this.injects().Viewer.getEditorMode()==="PREVIEW"){a.width=null;
a.w=null;
a.x=null
}this._changeSize(a)
},handleAppSettingsClose:function(a){this.handleClosePopup(a)
},handleClosePopup:function(a){return new Error("handleClosePopup: Override me!!!")
},_getMetadataParameter:function(b,c){var a=$$("meta["+(c||"name")+"='"+b+"']");
var e="";
if(a&&a.length>0){var d=a.get("content");
if(d.length>0){e=d[0]
}}return e
},handleAppShowPopup:function(a){this.handleOpenPopup(a)
},handleOpenPopup:function(e){e=e||{};
var a=this.injects().Viewer.getSiteNode();
var b,d,g,c,f;
switch(e.type){case"fixed":case"normal":d="wysiwyg.viewer.components.tpapps.TPAPopup";
g="wysiwyg.viewer.skins.TPAWidgetSkin";
f=this._data;
c={url:e.url,coordinates:Object.merge(this.DEFAULTS.POPUP_COORDINATES.NORMAL,(e.coordinates||{}))};
break;
case"modal":d="wysiwyg.viewer.components.Modal";
g="wysiwyg.viewer.skins.ModalSkin";
f="";
c={innerComponentArgs:{type:"wysiwyg.viewer.components.tpapps.TPAPopup",skin:"wysiwyg.viewer.skins.TPAWidgetSkin",dataItem:this._data,argObject:{url:e.url,coordinates:Object.merge(this.DEFAULTS.POPUP_COORDINATES.MODAL,(e.coordinates||{})),containingComponent:this}}};
break;
case"fullscreen":d="wysiwyg.viewer.components.Modal";
g="wysiwyg.viewer.skins.ModalSkin";
f="";
c={innerComponentArgs:{type:"wysiwyg.viewer.components.tpapps.TPAPopup",skin:"wysiwyg.viewer.skins.TPAWidgetSkin",dataItem:this._data,argObject:{url:e.url,coordinates:this.DEFAULTS.POPUP_COORDINATES.FULLSCREEN,containingComponent:this}}};
break
}b=W.Components.createComponent(d,g,f,c,null,null);
b.insertInto(a)
},handleSmRequestLogin:function(b){var a=this.injects().SiteMembers;
...
W.Classes.newTrait({name:"wysiwyg.viewer.components.traits.TPAHandlersBase",trait:{_appIsAlive:false,initialize:function(){if(!this._changeSize){return new Error("Must... Implement... _changeSize...")
}},handleAppIsAlive:function(){this._appIsAlive=true
},handleSiteInfo:function(l){var d=window.location.hash;
var b=this.injects().Utils.hash.getHashParts(d);
var g=location.protocol+"//"+location.host+location.pathname;
if(!this.injects().Viewer.isPublicMode()){if(W.Config.siteNeverSavedBefore()){g=null
}else{if(window.parent.editorModel){var k=window.parent.editorModel.publicUrl;
g=k||null
}}}else{if(window.publicModel){var h=window.publicModel.externalBaseUrl;
g=h||g
}}var m=(window.publicModel&&window.publicModel.favicon);
if(m){m=this.injects().Config.getMediaStaticUrl(m)+m
}var f=window.rendererModel?this.injects().Viewer.getSiteName():window.editorModel.metaSiteData.siteName;
var c=window.rendererModel?this.injects().Viewer.getSiteTitleSEO():window.editorModel.metaSiteData.title;
var j=this._getMetadataParameter("description");
var e=this._getMetadataParameter("keywords");
var a=this._getMetadataParameter("og:image","property");
l.callback({siteName:f,siteTitle:c,siteDescription:j,siteKeywords:e,pageTitle:(b&&b.title)||"",referrer:document.referrer,url:location.href,baseUrl:g,favIconUrl:m,thumbnail:a})
},_getMetadataParameter:function(b,c){var a=$$("meta["+(c||"name")+"='"+b+"']");
var e="";
if(a&&a.length>0){var d=a.get("content");
if(d.length>0){e=d[0]
}}return e
},handleAppSettingsChanged:function(e){var f=(this._editedComponent&&this._editedComponent.getApplicationId())||this.getApplicationId();
var b=(this.injects().Preview&&this.injects().Preview.getPreviewManagers().TPA.getComponentRegistrar())||this.injects().TPA.getComponentRegistrar();
var c=e.queryParams;
var d=e.compIds;
var a;
if(d){if(d instanceof Array){for(a in d){this._renderSingleComponent(b,d[a],c)
}}else{this._renderSingleComponent(b,d,c)
}}else{if(c){b.each(f,function(g){g.setQueryParams(c);
g.forceRender()
})
}else{b.each(f,function(g){g.forceRender()
})
}}}}});
...
},_onFocus:function(a){a.stopPropagation();
if(this.getState("mouse")=="selected"){this._onBlur(a);
return
...
},_onBlur:function(a){a.stopPropagation();
...
},_onOptionClick:function(d){d=d||{};
var b=d.target;
while(b!==this._skinParts.options&&b.hasClass(this.CLASS_NAME_OPTION_CONTAINER)==false){b=b.parentNode
}this._setSelected(b);
var c=location.hostname.split(".")[0];
var a=b.getAttribute("data-subdomain");
if(window.viewMode=="site"&&a&&c.toLowerCase()!=a.toLowerCase()){this._skinParts.select.fireEvent(Constants.CoreEvents.CHANGE,{value:a})
}this._onBlur(d)
},_onKeyPress:function(b){b.stopPropagation();
switch(b.key){case"up":var a=this._selectedOption.getPrevious();
if(!a){a=this._skinParts.options.getLast()
}this._setSelected(a);
b.preventDefault();
break;
case"down":this._setSelected(this._selectedOption.getNext());
b.preventDefault();
break;
case"enter":case"space":b.target=this._selectedOption;
this._onOptionClick(b);
break;
case"esc":case"tab":this._onBlur(b);
break
}},_setSelected:function(d){var a=this._skinParts.select;
var b=this._skinParts.options;
var c=this._skinParts.options.getElement("."+this.CLASS_NAME_SELECTED);
if(c){c.removeClass(this.CLASS_NAME_SELECTED)
}d=d||this._skinParts.options.getFirst();
d.addClass(this.CLASS_NAME_SELECTED);
a.setAttribute("data-subdomain",d.getAttribute("data-subdomain"));
a.setAttribute("data-languagecode",d.getAttribute("data-languagecode"));
a.set("html",d.get("html"));
var e=d.getPosition(b).y;
var f=b.getScroll().y;
if(e>b.getSize().y+f||e<0){b.scrollTo(0,e)
}this._selectedOption=d
},_populateDropDown:function(){var e,d,c,a,b;
this._skinParts.select.empty();
this._skinParts.options.empty();
for(e=0,a=this._menuData.length;
e<a;
e++){this._insertOption(this._menuData[e])
}this._updateSelection()
},_insertOption:function(d){var c=new Element("div");
var b=new Element("image");
var a=new Element("span",{html:d.languageLabel});
c.addClass(this.CLASS_NAME_OPTION_CONTAINER);
if(d.iconURL){b.setAttribute("src",d.iconURL);
b.inject(c)
}a.inject(c);
c.insertInto(this._skinParts.options);
c.setAttribute("data-subdomain",d.languageSubdomain);
c.setAttribute("data-languagecode",d.languageCode);
c.addEvent(Constants.CoreEvents.CLICK,this._onOptionClick)
},_getOptionBySubDomain:function(a){return this._skinParts.options.getElement('[data-subdomain="'+a+'"]')
},_changeLanguage:function(){var a=this._skinParts.select.getAttribute("data-subdomain");
var c=this._skinParts.select.getAttribute("data-languagecode");
var b=location.hostname.substring(location.hostname.indexOf("."));
if(window.userServer){window.userServer.setLanguage(c)
}else{LOG.reportError(wixErrors.USER_MANAGER_NOT_FOUND,"LanguagesDropDown","_changeLanguage")()
}location.href=location.protocol+"//"+a+b+location.pathname+location.search+location.hash
},_updateSelection:function(){var b=location.hostname.split(".")[0];
var a=this._getOptionBySubDomain(b);
this._setSelected(a)
},_removeAllDomEvents:function(){document.body.removeEvent(Constants.CoreEvents.CLICK,this._onBlur);
document.body.removeEvent(Constants.CoreEvents.KEY_DOWN,this._onKeyPress)
},dispose:function(){this._removeAllDomEvents();
this.parent()
}}});
