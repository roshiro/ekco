...
}var g=d.split(" ").filter(function(i){return !!i
});
if(g.length===1){var a="^#[\\dA-Fa-f]{3,8}$";
var f="^rgba?\\([\\d,]+\\)$";
var h="^[\\[\\{][\\w\\d_]+[\\]\\}]$";
var c=new RegExp(a+"|"+f+"|"+h);
var b=g[0];
...
},linkableComponentEditModeChanged:function(k,a,l){if(this._isDisposed){return
}var e=l?l:this._skinParts.link,j=false,c=false,g=this.getDataItem?this.getDataItem():a,b=g.get("linkType"),i=g.get("target"),f=W.Utils.linkUtils.Static.linkTarget.SAME_WINDOW,d=W.Utils.linkUtils.Static.linkType.WEBSITE,h=W.Utils.linkUtils.Static.linkType.EMAIL;
if(i===f&&b===d){j=true
}if(b===h){c=true
}if(k=="PREVIEW"){if(j){this._bindPageLeaveWarningToLink(e)
...
a.onSuccess=function(h,g){this._onSuccess(g,e)
}.bind(this);
...
},_getParamValue:function(h,b){var c=null;
if(h.defaultParam){var f=h.defaultParam;
return this._applyParamMutators(this._getParamValue(f,b),h)
}if(b&&b.get(h.id)){var j=b.getPropertySource(h.id);
c=b.get(h.id);
if(j=="theme"){c=this.injects().Theme.getProperty(c)
}c=this._castToType(c,h);
c=this._addExtraToParamByType(h,c,b);
return this._applyParamMutators(c,h)
}if(h.defaultTheme){var g=h.defaultTheme;
var k=W.Theme.getProperty(g);
k=this._castToType(k,h);
return this._applyParamMutators(k,h)
}if(h.sumParams){var e=h.sumParams;
if(e&&typeOf(e)=="array"&&e.length>0){var l=this._getParamValue(e[0],b);
if(l.add&&typeof l.add==="function"){for(var d=1;
d<e.length;
d++){l.add(this._getParamValue(e[d],b))
}}return this._applyParamMutators(l,h)
}}if(h.defaultValue){c=this._castToType(h.defaultValue,h);
return this._applyParamMutators(c,h)
}return null
...
},registerComponent:function(a,c){var b=a.getID();
if(this._registeredComponent[b]){LOG.reportError(wixErrors.COMPONENT_ALREADY_REGISTERED_IN_LIFECYCLE,"ComponentLifecycleManager","registerComponent",b);
return
}this._registeredComponent[b]={component:a,state:this.WAITING_FOR_RENDER};
if(c){this._renderImmediate(a)
}else{this._renderQueue.push(a)
}},_renderImmediate:function(a){var b=a.render();
...
while(this._renderQueue.length>0){var a=this._renderQueue[0];
var b=a.render();
this._handleRenderResult(a,b)
...
while(this._measureQueue.length>0){var a=this._measureQueue[0];
a.measure();
...
while(this._updateQueue.length>0){var a=this._updateQueue[0];
var b=a.update();
this._handleUpdateResult(a,b)
...
},addToRenderQueue:function(a){var b=a.getID();
if(!this._registeredComponent[b]){this.registerComponent(a)
}else{if(this._registeredComponent[b].state==this.WAITING_FOR_RENDER){return
}else{this._removeComponentFromCurrentQueue(a);
this._updateComponentState(a,this.WAITING_FOR_RENDER);
this._renderQueue.push(a)
}}},addToMeasureQueue:function(a){var b=a.getID();
if(!this._registeredComponent[b]){this.registerComponent(a)
}else{if(this._registeredComponent[b].state==this.WAITING_FOR_MEASURE){return
}else{this._removeComponentFromCurrentQueue(a);
this._updateComponentState(a,this.WAITING_FOR_MEASURE);
this._measureQueue.push(a)
}}},_updateComponentState:function(a,c){var b=a.getID();
if(!this._registeredComponent[b]){LOG.reportError(wixErrors.COMPONENT_NOT_REGISTERED_IN_LIFECYCLE,"ComponentLifecycleManager","_updateComponentState",b);
return
}this._registeredComponent[b].state=c
...
return
}}},_getComponentState:function(a){var b=a.getID();
return this._registeredComponent[b].state
